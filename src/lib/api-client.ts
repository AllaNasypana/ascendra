...
export const api = {
  auth: {
    ...
    register: (name: string, email: string, password: string, role: PublicUser['role']) =>
      request<{ user: PublicUser }>('/api/auth/register', {
        method: 'POST',
        body: JSON.stringify({ name, email, password, role }),
      }),
    me: (userId: string) =>
      request<{ user: PublicUser }>(`/api/auth/me?${CURRENT_USER_ID}=${userId}`),
  },
  ...
  vms: {
    list: (params?: { ownerId?: string }) => {
      ...
      return request<{ vms: VM[]; users: PublicUser[]; templates: VMTemplate[] }>(
        `/api/vms${search}`,
      );
    },
    get: (id: string) =>
      request<{ vm: VM; owner?: PublicUser; template?: VMTemplate }>(`/api/vms/${id}`),
